...
class gripper:
    ...
    def gripper_angle(self, angle):
        self.robot.addCommand(f"(gripper-angle {self.name} {angle})")
...
class cubeSolver:
    ...
    def adjustPosition(self):
        self.gripperL.gripper_angle(30)
        self.gripperR.gripper_angle(30)
        self.gripperR.robot.addCommand('(send *ri* :angle-vector (send *robot* :angle-vector) 500)')
        self.gripperR.robot.addCommand('(send *ri* :wait-interpolation)')
        self.gripperL.gripper_angle(25)
        self.gripperR.gripper_angle(25)
        self.gripperR.robot.addCommand('(send *ri* :angle-vector (send *robot* :angle-vector) 500)')
        self.gripperR.robot.addCommand('(send *ri* :wait-interpolation)')
